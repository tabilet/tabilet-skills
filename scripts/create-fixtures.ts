...
for (const [name, count] of [['active', 120], ['legacy', 1], ['retired', 0], ['linked', 0], ['draft', 1], ['new', 0]] as const) {
  const files = activeFixture(count), project = join(state.root, name); projects[name] = project;
  if (name === 'new') { files.clear(); files.set('AGENTS.md', '# New project\n'); }
  if (name === 'legacy') { for (const [path, body] of [...files]) { if (path.startsWith('tabilet/')) { files.delete(path); files.set(path.slice('tabilet/'.length), body); } } files.set('memory-bank/status.md', '| Legacy | [ ] |'); files.delete('memory-bank/lessons.md'); }
  if (name === 'retired') {
    files.set('tabilet/memory-bank/milestone.md', '# Milestones\n[History](../docs/history/index.md)\n');
    const body = cases.find((c: { name: string }) => c.name === 'cancelled').text;
    files.set('tabilet/docs/history/index.md', '| M01 | cancelled | 2026-09-12 | [Record](status-M01.md) | Owner approved |\n');
    files.set('tabilet/docs/history/status-M01.md', body);
    files.set('tabilet/docs/history/knowledge.md', '# Knowledge history\nPreserved learning.\n');
    files.set('tabilet/docs/archive-A01.md', '# Frozen context\nOriginal product facts.\n');
  }
  if (name === 'linked') {
    ...
    files.set('tabilet/memory-bank/milestone.md', '| M01 | [Canonical](../../../canonical/status-M01.md) |\n');
  }
